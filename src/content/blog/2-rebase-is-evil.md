...
In a scenario with no conflict, if you want to rebase your feature branch commits on top of main, you will NEED to force push. That's already super dumb workflow.
...
You will then need to resolve conflicts for **every commit**. That's a shit ton of work for a shit ton of nothing. Zero productivity boost and zero gains from it.
...
Actually that's really a **dumb** idea. You will rebase n commits from your branch, but you will merge your branch only assuming the **latest commit** compiles and passes tests. The conflicts you resolved represent states of the project that never existed and were never tested.
